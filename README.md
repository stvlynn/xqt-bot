...
| var | `LLM_BASE_URL` / `LLM_MODEL` | 模型端点与型号。任何 OpenAI 兼容端点均可（OpenAI、DeepSeek、Moonshot 开放平台 `https://api.moonshot.cn/v1` 等）。**注意：Kimi 会员编程套餐的 `kimi-for-coding` 端点不可用**——它按 User-Agent 白名单限制只允许编程 Agent（Kimi CLI / Claude Code 等）调用，通用程序会收到 403 |
